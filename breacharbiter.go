...
func (b *breachArbiter) Start() error {
	...
	for i, chanState := range activeChannels {
		channel, err := lnwallet.NewLightningChannel(nil, b.notifier, chanState)
		if err != nil {
...
out:
	for {
		select {
		case pendingRetribution := <-b.breachedContracts:
			...
			breachTXID := &pendingRetribution.commitHash
			confChan, err := b.notifier.RegisterConfirmationsNtfn(breachTXID, 1)
			if err != nil {
				brarLog.Errorf("unable to register for conf updates for txid: "+
					"%v, err: %v", breachTXID, err)
			...
			if err := b.retributionStore.Add(pendingRetribution); err != nil {
			...
			b.wg.Add(1)
			go b.exactRetribution(confChan, pendingRetribution)

			delete(b.breachObservers, pendingRetribution.chanPoint)
...
func (b *breachArbiter) breachObserver(contract *lnwallet.LightningChannel,
	...
	case breachInfo := <-contract.ContractBreach:
		...
		localWitnessType := lnwallet.CommitmentNoDelay
		...
		remoteWitnessType := lnwallet.CommitmentRevoke
...
func (b *breachArbiter) exactRetribution(confChan *chainntnfs.ConfirmationEvent,
	...
	// Finally, broadcast the transaction, finalizing the channel's
	...
	case _, ok := <-confChan.Confirmed:
		...
		revokedFunds := ret.revokedOutput.amt
		totalFunds := revokedFunds + ret.selfOutput.amt

		brarLog.Infof("Justice for ChannelPoint(%v) has "+
			"been served, %v revoked funds (%v total) "+
			"have been claimed", ret.chanPoint,
